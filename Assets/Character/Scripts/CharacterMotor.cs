...
public class CharacterMotor : MonoBehaviour, IPausable
{
    ...
    protected virtual void Update()
    {
        ...
        Vector2 cameraInput = Effectable.Effect_CameraInput(GetCameraInput()) * Time.deltaTime;
    ...
    protected virtual void FixedUpdate()
    {
        ...
        else
        {
            ...
            if (jumpRequested)
            {
                ...
                JumpTargetY = transform.position.y + Effectable.Effect_JumpHeight(Config.JumpHeight);
            ...
            if (IsJumping)
            {
                ...
                velocity.y = Mathf.Lerp(Mathf.Max(0f, velocity.y), Effectable.Effect_JumpVelocity(Config.JumpVelocity), Config.JumpVelocityBlend);
            ...
            {
                CharacterRB.AddForce(Vector3.down * Effectable.Effect_FallForce(Config.FallForce), ForceMode.Impulse);
            }
            else
            {
    ...
    public float CurrentSpeed
    {
        get
        {
            return IsGrounded ? Effectable.Effect_GroundSpeed(IsRunning ? Config.RunSpeed : Config.WalkSpeed) : 
                                Effectable.Effect_InAirSpeed(Config.AirControl ? Config.InAirSpeed : 0);
        }
    }
